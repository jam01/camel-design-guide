...
| **The reset goes FIRST in the catch body, not last.** Measured: with it first, a compensation route called from the body has its own clause fire; with it last, that clause never fires, the compensation's failure escapes unmapped, and the reset itself never runs. Also with it first, a throw from the body escapes unclaimed and a **calling** route maps it; without, it cannot be mapped. What it does not buy: a called route that *handles* its failure still stops the caller, by the ordinary ownership rule. | `ContinuePlacementProbe` |

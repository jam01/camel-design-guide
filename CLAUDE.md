...
| **The claim comes from a failure reaching a handler, not from having one.** A stage that catches its own failure in an internal `doTry` and converts it to a value leaves the exchange **unclaimed** — measured on a *transacted* stage, after which a later failure in the caller was still fully mapped by the edge clause. So edge-only translation IS reachable; what forecloses it is a clause firing, not a transaction existing. | `ContainedFailureProbe` |
